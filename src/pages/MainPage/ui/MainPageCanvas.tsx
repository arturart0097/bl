    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      >
        <Layer>
                ...
                {iconImage && (
                  <Image
                    ...
                    {...(b.color && !showColorGlow ? {} : {
                      ...
                      shadowColor: b.color ? b.color : (!selectedRGB ? undefined : rgbCycle(blinkTime)),
                      globalCompositeOperation: "lighter"
                    })}
                    listening={false}
                  />
          ...
          {lightStreams.map((stream) => {
            if (!roads) return null;
            const road = roads.find((r) => r.id === stream.road);
            if (!road) return null;
          ...
          {/* --- FOOTSTEPS --- */}
          {footstepsToShow.map((step, i) => {
            // Плавна поява: кожна пара кроків проявляється по черзі
            const appearDuration = 600; // мс
            const delayBetweenPairs = 400; // мс
            // Пара = два кроки (ліва/права)
            ...
            return (
              ...
              >
                ...
                <Ellipse
                  ...
                  globalCompositeOperation="lighter"
                />
                {/* Носок (маленький овал, трохи попереду та збоку) */}
                {/* <Ellipse
                  x={step.toeOffsetX || 3}
                  y={step.toeOffsetY || 0}
                  radiusX={1.2}
                  ...
                  shadowBlur={8}
                  shadowColor={footColor}
                  opacity={step.toeAlpha || 0.7}
                  listening={false}
                  globalCompositeOperation="lighter"
                  rotation={step.toeAngle || 0}
                /> */}
                {/* Пальці (2 маленькі точки) */}
                {/* <Ellipse
                  x={(step.toeOffsetX || 3) + 1.2}
                  y={(step.toeOffsetY || 0) - 0.7}
                  radiusX={0.5}
                  radiusY={0.5}
                  fill={footColor}
                  opacity={0.4}
                ...
                />
                <Ellipse
                  x={(step.toeOffsetX || 3) + 2.1}
                  y={(step.toeOffsetY || 0) + 0.5}
                  radiusX={0.4}
                  radiusY={0.4}
                  fill={footColor}
                  opacity={0.25}
